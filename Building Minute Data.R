...
data_set$hour <- replace(data_set$hour, data_set$hour==0, 1)
data_set$hour <-(data_set$hour-1)

data_ts <- msts(data_set$load, seasonal.periods=c(60,	1440,	10080,	525960), start=2017)
data.fit <- tbats(data_ts)

dat <- c(ISOdate(2017, 1, 3), ISOdate(2017, 1, 1))
startTime <- difftime(dat[1], dat[2], units="hours")
orig_data <- ts(load_data[, "System_Load"], start=c(2017, as.numeric(startTime)), frequency=24*365)